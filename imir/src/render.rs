...
/// # Errors
///
/// Returns [`AppError`] when target_user is empty or include_private is invalid.
...
) -> Result<ProfileInputs, AppError,>
{
    ...
    let branch = branch_name.filter(|s| !s.is_empty(),).unwrap_or("ci/metrics-refresh-profile",);

    let path = target_path.filter(|s| !s.is_empty(),).unwrap_or("metrics/profile.svg",);

    let artifact = temp_artifact.filter(|s| !s.is_empty(),).unwrap_or(".metrics-tmp/profile.svg",);

    let tz = time_zone.filter(|s| !s.is_empty(),).unwrap_or("Asia/Ho_Chi_Minh",);

    let name = display_name.filter(|s| !s.is_empty(),).unwrap_or("profile",);

    let private_normalized = include_private
        .unwrap_or("",)
        .to_lowercase();
    ...
    let (
        ...
        _ => return Err(AppError::validation("include_private must be a boolean value",),),
    };
...
/// * `target_owner` - Repository owner (optional, uses GITHUB_REPOSITORY owner if empty)
...
/// # Errors
///
/// Returns [`AppError`] when target_repo is empty or contributors_branch is invalid.
...
) -> Result<RepositoryInputs, AppError,>
{
    ...
    if contrib_branch.contains(char::is_whitespace,) {
        return Err(AppError::validation("contributors_branch cannot contain whitespace",),);
    }
...
mod tests
{
    ...
    fn normalize_profile_inputs_with_custom_values()
    {
        ...
        assert_eq!(result.repositories_affiliations, "owner, collaborator, organization_member");
    }
